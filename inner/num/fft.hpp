...
namespace inner {
  namespace fft {
    ...
    il std::vector<cpx> roots = {{0, 0}, {1, 0}};

    il void ensure_capacity(int min_capacity) {
    ...
    il void fft(std::vector<cpx> &z, bool inverse) {
      int n = z.size();
      assert((n & (n - 1)) == 0);
      ensure_capacity(n);
    ...
    il std::vector<int> multiply_bigint(const std::vector<int> &a, const std::vector<int> &b, int base) {
    ...
    il std::vector<int> multiply_mod(const std::vector<int> &a, const std::vector<int> &b, int m) {
