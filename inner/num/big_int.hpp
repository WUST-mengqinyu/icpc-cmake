    ...
    struct bigint {
      ...
      friend bigint sqrt(const bigint &a1) {
        ...
        for (int j = n / 2 - 1; j >= 0; j--) {
          for (;; --q) {
            bigint r1 = (r - (res * 2 * base + bigint(q)) * bigint(q)) * bigint(base) * bigint(base) +
                        bigint(j > 0 ? (long long) a.z[2 * j - 1] * base + a.z[2 * j - 2] : 0);
            if (r1 >= bigint(0)) {
              r = r1;
              break;
            }
      ...
      bigint operator*(const bigint &v) const {
        ...
        res.z = ::inner::fft::multiply_bigint(convert_base(z, base_digits, fft_base_digits),
