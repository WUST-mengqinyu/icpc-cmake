...
template <typename T> static inline T __testlib_abs(const T &x) {
  return x > 0 ? x : -x;
}

template <typename T> static inline T __testlib_min(const T &a, const T &b) {
  return a < b ? a : b;
}

template <typename T> static inline T __testlib_max(const T &a, const T &b) {
  return a > b ? a : b;
}
...
public:
  ...
  double next(double from, double to) {
    if (from > to)
      __testlib_fail(
          "random_t::next(double from, double to): from can't not exceed to");
  ...
  long long wnext(long long n, int type) {
    ...
    if (abs(type) < random_t::lim) {
      ...
      return __testlib_min(__testlib_max((long long)(double(n) * p), 0LL),
                           n - 1LL);
    }
  }
  ...
  template <typename T, typename E> std::vector<E> perm(T size, E first) {
    if (size <= 0)
      __testlib_fail(
          "random_t::perm(T size, E first = 0): size must be positive");
    std::vector<E> p(size);
    E current = first;
    for (T i = 0; i < size; i++)
      p[i] = current++;
  ...
  template <typename T> std::vector<T> partition(int size, T sum, T min_part) {
    ...
    if (*std::min_element(result.begin(), result.end()) < min_part)
      __testlib_fail("random_t::partition: partition min is expeced to be to "
...
enum TTestlibMode { _unknown, _checker, _validator, _generator, _interactor };
...
struct ValidatorBoundsHit {
  ...
  ValidatorBoundsHit merge(const ValidatorBoundsHit &validatorBoundsHit) {
    return ValidatorBoundsHit(__testlib_max(minHit, validatorBoundsHit.minHit),
  ...
  void writeTestOverviewLog() {
    if (!_testOverviewLogFileName.empty()) {
      ...
      if (NULL == testOverviewLogFile)
        __testlib_fail(
            "Validator::writeTestOverviewLog: can't test overview log to (" +
            fileName + ")");
...
static inline long long stringToLongLong(InStream &in, const char *buffer) {
  ...
  if (equals(retval, buffer))
    ...
    in.quit(_pe,
            ("Expected int64, but \"" + __testlib_part(buffer) + "\" found")
                .c_str());
}
...
bool has_opt(const std::string key) { return __testlib_opts.count(key) != 0; }

template <typename T> T opt(std::false_type, int index);

template <> std::string opt(std::false_type, int index) {
  return __testlib_indexToArgv(index);
}

template <typename T> T opt(std::true_type, int index) {
  return T(optValueToLongDouble(__testlib_indexToArgv(index)));
}

template <typename T, typename U> T opt(std::false_type, U, int index) {
  return opt<T>(std::is_floating_point<T>(), index);
}

template <typename T> T opt(std::true_type, std::false_type, int index) {
  return optValueToIntegral<T>(__testlib_indexToArgv(index), false);
}

template <typename T> T opt(std::true_type, std::true_type, int index) {
  return optValueToIntegral<T>(__testlib_indexToArgv(index), true);
}

template <> bool opt(std::true_type, std::true_type, int index) {
...
template <typename T> T opt(std::false_type, const std::string &key);

template <> std::string opt(std::false_type, const std::string &key) {
  return __testlib_keyToOpts(key);
}

template <typename T> T opt(std::true_type, const std::string &key) {
...
template <> bool opt(std::true_type, std::true_type, const std::string &key) {
  if (!has_opt(key))
    return false;
